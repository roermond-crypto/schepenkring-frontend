...
export default function FleetManagementPage() {
  ...
  const deriveStats = (yachts: YachtListItem[]): FleetStats => ({
    total: yachts.length,
  ...
  const getYachtName = (yacht: YachtListItem): string => {
  ...
  const getYachtStatus = (yacht: YachtListItem): string => {
  ...
  const getClientContractStatusText = (summary: LatestSignhostSummary) => {
    const statusMap: Record<string, string> = {
      pending_review:
        t?.contract?.pendingReview || "Pending broker review",
      waiting_invite:
        t?.contract?.waitingInvite || "Waiting for Signhost invite",
      signing: t?.contract?.signing || "Ready to sign",
      signed: t?.contract?.signed || "Signed",
      failed: t?.contract?.failed || "Needs attention",
    };

    return statusMap[summary.status] || statusMap.pending_review;
  };

  const getClientContractActionLabel = (summary: LatestSignhostSummary) => {
    if (summary.status === "signing") {
      return t?.actions?.signContract || "Sign Contract";
    }

    return t?.actions?.openContract || "Open Contract";
  };

  const getClientContractTone = (summary: LatestSignhostSummary) => {
                  ...
                  {isClientRole ? (
                    <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-3">
                      <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                        {t?.contract?.label || "Contract Signing"}
                      </p>
                      <div
                        className={cn(
                          "mt-2 inline-flex rounded-full border px-3 py-1 text-[9px] font-black uppercase tracking-widest",
                          getClientContractTone(yacht.latest_signhost ?? normalizeLatestSignhost(null)),
                        )}
                      >
                        {getClientContractStatusText(
                  ...
                  <div className="pt-4 border-t border-slate-100 mt-auto space-y-2">
                    ...
                    isClientContractActionable(
                      ...
                      <button
                        onClick={() =>
                          router.push(
                            `/${locale}/dashboard/${role}/yachts/${yacht.id}?step=6`,
                          )
                        }
                        className="w-full rounded-lg bg-[#003566] px-4 py-2 text-[9px] font-black uppercase tracking-widest text-white transition-colors hover:bg-blue-800"
                      >
              ...
              {/* VESSEL */}
                ...
                <div className="col-span-2 flex items-center gap-2">
                  ...
                  {isClientRole ? (
                    <span
                      className={cn(
                        "inline-flex rounded-full border px-3 py-1 text-[9px] font-black uppercase tracking-widest",
                        getClientContractTone(
                          yacht.latest_signhost ?? normalizeLatestSignhost(null),
                        ),
                      )}
                    >
                ...
                <div className="col-span-2 flex items-center justify-end gap-2">
                  ...
                  isClientContractActionable(
                    ...
                    <button
                      ...
                      className="rounded-md bg-[#003566] px-3 py-2 text-[9px] font-black uppercase tracking-widest text-white transition-colors hover:bg-blue-800"
                      title={getClientContractActionLabel(
                        yacht.latest_signhost ?? normalizeLatestSignhost(null),
                      )}
                    >
                      {getClientContractActionLabel(
